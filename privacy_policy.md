...
Which features of the Extension you have enabled or disabled (e.g., "Percentage of Column Total," "Sticky Report Header," "GA4 Debugging").
...
4. Feature-Specific Data Handling

GA4 Debugging Feature: The "Enable GA4 Debugging" feature, when activated by you, works by modifying network requests to Google's own analytics services (googletagmanager.com). This is done locally to enable Google Analytics' DebugView for your own analysis and troubleshooting. This feature does not send any data to the developer of GA4 Optimizer or any unrelated third parties.
...
* clipboardWrite: For the "Click to Copy Cell" feature described in Section 4.
* declarativeNetRequest: For the "Enable GA4 Debugging" feature described in Section 4.
* Host Permissions (https://analytics.google.com/analytics/web/* and <all_urls>):
* https://analytics.google.com/analytics/web/*: Essential for the Extension to operate on Google Analytics 4 pages.
* <all_urls>: Primarily required for the "Enable GA4 Debugging" feature. This allows the Extension to modify network requests to Google Tag Manager (googletagmanager.com) on any website you choose to debug, which is necessary for activating GA4's DebugView on those sites. The web_accessible_resources setting in the manifest also uses this match to ensure the Extension's packaged help and onboarding pages (documentation.html, onboarding.html) can be opened correctly in your browser.
